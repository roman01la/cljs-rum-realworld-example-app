...
(rum/defc Register < rum/reactive
  ...
  (let [form-data (rum/cursor-in form-state [:data])
        ...
        validators {:username [[v/not-empty? "Please enter username"]]
                    :email [[v/not-empty? "Please enter email"]
                             [v/email? "Invalid Email"]]
                    :password [[v/not-empty? "Please enter password"]]}
        handle-submit (fn [e]
                        (.preventDefault e)
                        (when (v/validate! form-data form-errors validators)
                          (citrus/dispatch! r :user :register {:username (:username @form-data)
